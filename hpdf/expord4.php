...
try {
    ob_start();
    include dirname(__FILE__).'/expord4b.php';
    $content = ob_get_clean();
    $newfile = dirname(__FILE__).'/ordfiles/ord4-'.trim($ordcli).'.pdf';
    $higPage = 260;
    $higPage = $higPage + ( count($order) * 38 );
    $html2pdf = new Html2Pdf('P',array($higPage,'210'),'en', false, 'ISO-8859-15',array(0, 0, 0, 0));
    //$html2pdf->pdf->SetDisplayMode('fullpage');
    $html2pdf->setDefaultFont('helvetica');
    $html2pdf->writeHTML($content);
    $html2pdf->output($newfile,'Fi');
    $datrfq = Array ( "ord_genpdf" => 1 );
    $db->where ('ord_id', $order_id);
